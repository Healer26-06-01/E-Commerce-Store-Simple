...
const Basket = () => {
    ...
    const renderCart = () => {
        if (cartItems.length > 0) {
            ...
            return <div>Giỏ hàng hiện tại trống</div>;
        }
    };
    ...
    return (
        <BasketContainer>
            <BasketTitle>Giỏ hàng</BasketTitle>
            <BasketButton onClick={() => navigate("/checkout")}>Thủ tục thanh toán</BasketButton>
            ...
            <BasketButton onClick={() => setCartItems(clearBasket())}>Xóa hết</BasketButton>
            <BasketTotal>Tổng tiền: {formatNumber(renderTotal())}</BasketTotal>
        </BasketContainer>
    );
};
